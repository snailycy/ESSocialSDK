...
public class QQSSOProxy {
    ...
    public static void getUserInfo(Context context, String appId, SocialToken socialToken, IUiListener listener) {
        //这里只是为了在刷新用户信息的时候，设置一下全局变量。场景：APP升级安装，没有重新登录过程，调用该接口刷新用户信息，
        //如果没有该过程刷新Global.getContext()等常量，会出现空指针
        Tencent tencent = getTencentInstance(context, appId);
        tencent.setAccessToken(socialToken.getToken(), String.valueOf(socialToken.getExpiresTime()));
        tencent.setOpenId(socialToken.getOpenId());
